  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-slate-900 mb-2">Review Data Input</h2>
      ...
      {/* Criteria Summary */}
      <Card className="p-6">
        <h3 className="text-lg font-semibold text-slate-900 mb-4">Kriteria Penilaian</h3>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="text-left py-3 px-4 font-medium text-slate-700">No</th>
                <th className="text-left py-3 px-4 font-medium text-slate-700">Nama Kriteria</th>
                <th className="text-left py-3 px-4 font-medium text-slate-700">Bobot</th>
                <th className="text-left py-3 px-4 font-medium text-slate-700">Tipe</th>
              </tr>
            </thead>
            <tbody>
              {criteria.map((criterion, index) => (
                <tr key={criterion.id} className="border-b border-slate-100">
                  ...
                  <td className="py-3 px-4 font-medium text-slate-900">{criterion.name}</td>
                  <td className="py-3 px-4 text-slate-600">{criterion.weight.toFixed(3)}</td>
      ...
      <Card className="p-6">
        <h3 className="text-lg font-semibold text-slate-900 mb-4">Data Alternatif</h3>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="text-left py-3 px-4 font-medium text-slate-700">No</th>
                <th className="text-left py-3 px-4 font-medium text-slate-700">Nama Alternatif</th>
                {criteria.map(criterion => (
                  <th key={criterion.id} className="text-left py-3 px-4 font-medium text-slate-700">
                    {criterion.name}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {alternatives.map((alternative, index) => (
                <tr key={alternative.id} className="border-b border-slate-100">
                  <td className="py-3 px-4 text-slate-600">{index + 1}</td>
                  <td className="py-3 px-4 font-medium text-slate-900">{alternative.name}</td>
                  {criteria.map(criterion => (
