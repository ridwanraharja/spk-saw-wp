...
export const calculateSAW = (criteria: Criterion[], alternatives: Alternative[]): SAWResult[] => {
  ...
  const preferenceValues = normalizedMatrix.map(row => 
    row.reduce((sum, normalizedValue, j) => 
      sum + (normalizedValue * criteria[j].weight), 0
    )
  );
...
export const calculateWP = (criteria: Criterion[], alternatives: Alternative[]): WPResult[] => {
